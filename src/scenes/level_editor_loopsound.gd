...
func _on_editor_note_hit(data):
	var arr = Difficulty.get_chart_notes().filter(func(note): return LevelEditor.song_position_offset >= note.timestamp and note.note_modifier != LevelEditor.NOTETYPE.GHOST)
	var idx: int = arr.size()-1
	
	if current_bank.size() > 0 and data.note_modifier != LevelEditor.NOTETYPE.GHOST:
		var current_bank_index = wrapi(idx, 0, current_bank.size())
