...
func _update_child_order():
	for child in get_children():
		var idx: int = Config.keyframes['sound_loop'].find(child.data)
		if child.get_index() != idx: move_child(child, idx)

func _add_loopsound(data: Dictionary):
	LevelEditor.add_single_keyframe(data, self, editor_keyframe_prefab)
	if Config.keyframes['sound_loop'].find(data) < 0:
		Config.keyframes['sound_loop'].append(data)
		Config.keyframes['sound_loop'].sort_custom(func(a, b): return a['timestamp'] < b['timestamp'])
	
	_update_child_order()

func _remove_loopsound(data: Dictionary):
	_update_child_order()
	
	var idx: int = Config.keyframes['sound_loop'].find(data)
...
func create_keyframe(keyframe_data: Dictionary):
	...
	if LevelEditor.create_new_keyframe("sound_loop", new_keyframe_data, timestamp):
		Global.undo_redo.create_action("Add Sound Loop")
...
func remove_keyframe(data: Dictionary):
	...
	Global.undo_redo.create_action("Remove Sound Loop")
