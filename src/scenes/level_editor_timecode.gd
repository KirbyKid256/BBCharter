...
func _process(_delta):
	...
	current_time_min = int(fmod(LevelEditor.song_position_offset, 60 * 60) / 60)
	...
	total_time_msec = snappedi(fmod(LevelEditor.song_length, 1) * 1000, 1)
	total_time_sec = int(fmod(LevelEditor.song_length, 60))
	total_time_min = int(fmod(LevelEditor.song_length, 60 * 60) / 60)
	
	offset_time_msec = snappedi(fmod(Config.settings['song_offset'], 1) * 1000, 1)
	offset_time_sec = int(fmod(Config.settings['song_offset'], 60))
	offset_time_min = int(fmod(Config.settings['song_offset'], 60 * 60) / 60)
